...
attenuation1 = zeros(3,  floor(length(kgrid.t_array) / 2) + 1);
cp1 = zeros(3, floor(length(kgrid.t_array) / 2) + 1);

figure(1);
AttPlots=tiledlayout(2,3,'TileSpacing','Tight');

figure(2);
CpPlots=tiledlayout(2,3,'TileSpacing','Tight');

medium.density = 0*kgrid.x +1;
for L = [20,40,60,80,120,240]
    medium.alpha_L=L;
    figure(1)
    nexttile()
    figure(2)
    ...
    sensor_data1 = kspaceFirstOrder1D(kgrid, medium, source, sensor, 'PlotSim', false);
    [~, as11, ps11] = spect(sensor_data1(1, :), 1/kgrid.dt);
    [f, as21, ps21] = spect(sensor_data1(2, :), 1/kgrid.dt);
    attenuation1(loop, :) = -20 * log10(as21 ./ as11) ./ d_cm;
    cp1(loop, :) = 2 .* pi .* f .* d ./ (unwrap(ps11) - unwrap(ps21));
    end
    f_max=50;
    figure(1)
    ds = 8;
    ...
    hold on
    plot(f(1:ds:end) .* 1e-6, attenuation1(:, 1:ds:end), 'ro');
    drawnow
    figure(2)
    plot(f(1:ds:end) .* 1e-6, cp(:, 1:ds:end), 'ko', f .* 1e-6, cp_kk, 'k-');
    hold on
    plot(f(1:ds:end) .* 1e-6, cp1(:, 1:ds:end), 'ro');
    drawnow
end
figure(1)
nexttile(1)
