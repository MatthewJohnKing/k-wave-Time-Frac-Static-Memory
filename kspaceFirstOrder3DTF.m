function [sensor_data, mem_usage] = kspaceFirstOrder3DTF(kgrid, medium, source, sensor, varargin) %#ok<STOUT>
% kspaceFirstOrder1DTFis a 3D time-domain simulation of wave
% propagation. Adapted from the kspacefirstorder3D k-wave code.
%
% ADAPTIONS:
...
%     lossless cases.
%     This code is yet to undergo testing to ensure the results are
%     correct. Testing must be implemented! There is no obvious reason why
%     this code should not run successfully and correctly, however
%     recomended comparisons include making use kspaceFirstorder3Dalpha to
%     test is both give the same results for a domain that varies alpha
%     properly, and direct comparisons again kspaceFirstOrder3D with a
%     varied alpha input that is only different in the corner by setting
%     medium.alpha_power(1,1,1)=... . 
...
% USAGE:
%     sensor_data = kspaceFirstOrder3DTF(kgrid, medium, source, sensor)
%     sensor_data = kspaceFirstOrder3DTF(kgrid, medium, source, sensor, ...) 
%
...
switch equation_of_state
    case 'absorbing'
        if isscalar(medium.alpha_power)
            L=medium.alpha_L;
            [s,A]=BirkSongpts(L,medium.alpha_power-1);
            % s=permute(s,[2,3,1]);
            expsdt=zeros(L,1);
            onemexp=zeros(L,1);
            ovs2=zeros(L,1);
            ...
            psi = zeros(medium.alpha_L,kgrid.Nx, kgrid.Ny,kgrid.Nz);
        else
            uniquePowerValues = unique(medium.alpha_power(:));
            s=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
            A=castZeros([medium.alpha_L,kgrid.Nx,kgrid.Ny,kgrid.Nz]);
            ovs2=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
            expsdt=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
            onemexp=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
            ...
            for yInd = 1:length(uniquePowerValues)
                ...
                s(:,idx) = sT1(:,idx);
                A(:,idx) = AT1(:,idx);
                absorb_TF_tau(idx)=2* c0 .* (medium.alpha_coeff(idx)) ./ sin((uniquePowerValues(yInd)-1)*pi/2);
                
                for zind=1:medium.alpha_L
                    expsdtT(zind)=double(exp(-kgrid.dt.*vpa(sT(zind)).^2));
                    onemexpT(zind)=double(1-exp(-kgrid.dt.*vpa(sT(zind)).^2));
                    ovs2T(zind)=double(vpa(sT(zind)).^-2);
                end

                expsdtT1=repmat(expsdtT.', [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                expsdt(:,idx)=expsdtT1(:,idx);

                onemexpT1=repmat(onemexpT.', [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                onemexp(:,idx)=onemexpT1(:,idx);
    ...
    if ~flags.nonlinear
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                psi=psiupdate(psi,expsdt,onemexp,ovs2,rho0,duxdx,duydy,duzdz);
                p=pundate(medium.alpha_power,c0,rhox,rhoy,absorb_TF_tau,A,psi);
...
function psi=psiupdate(psi,expsdt,onemexp,ovs2,rho0,duxdx,duydy,duzdz)
B=permute(rho0 .* (duxdx + duydy + duzdz),[4,1,2,3]);
psi = expsdt .* psi - (onemexp.*ovs2.* B);
end

function p=pundate(power,c0,rhox,rhoy,absorb_TF_tau,A,psi)
if isscalar(power)
