...
%
% Changes made adjust checks for medium.alpha_power and medium.alpha_coeff
% allowing both to be the size of the space, as long as they are both the
% same size. Additionally, alpha_L is now allowed for the Time Fraction
% computation.
%
...
elseif flags.elastic_code
    ...
    checkFieldNames(medium, {'sound_speed', 'sound_speed_ref', 'density',...
        'alpha_coeff', 'alpha_power', 'alpha_mode', 'alpha_filter', 'alpha_sign', 'BonA', 'alpha_L','wall'});
...
if isfield(medium, 'alpha_coeff') || isfield(medium, 'alpha_power')
    ...
    if flags.axisymmetric || (isfield(medium, 'alpha_mode') && strcmp(medium.alpha_mode, 'stokes'))
        ...
        if numel(medium.alpha_power) ~= 1 
            if  isscalar(unique(medium.alpha_power)) 
                medium.alpha_power=unique(medium.alpha_power);
            elseif kgrid.dim==1 && numel(medium.alpha_power) == kgrid.Nx
        ...
        if ~isreal(medium.alpha_coeff) || (max(max(max(medium.alpha_power >= 3))) || max(max(max(medium.alpha_power < 0))))
