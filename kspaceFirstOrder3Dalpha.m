function [sensor_data, mem_usage] = kspaceFirstOrder3Dalpha(kgrid, medium, source, sensor, varargin) %#ok<STOUT>
% kspaceFirstOrder3Dalpha is a 3D time-domain simulation of wave
...
%     recomended comparisons include making use kspaceFirstorder3DTF to
...
% USAGE:
%     sensor_data = kspaceFirstOrder3Dalpha(kgrid, medium, source, sensor)
%     sensor_data = kspaceFirstOrder3Dalpha(kgrid, medium, source, sensor, ...) 
%
...
% Make sure the alpha mediums are the same size
...
if max(size(medium.alpha_coeff)~=size(medium.alpha_power))
    medium.alpha_coeff=castZeros([kgrid.Nx,kgrid.Ny,kgrid.Nz])+medium.alpha_coeff;
    medium.alpha_power=castZeros([kgrid.Nx,kgrid.Ny,kgrid.Nz])+medium.alpha_power;
end
...
if ~isscalar(medium.alpha_power)
    ...
    Locations=castZeros([kgrid.Nx,kgrid.Ny,kgrid.Nz,2]);
    for yInd=1:length(uniquepowers)
        ...
        for zInd=1:length(uniquecoeffs)
            numuniquepairs=numuniquepairs+1;
            Locations(:,:,:,numuniquepairs)=min( uniquepowers(yInd)==medium.alpha_power , uniquecoeffs(zInd)==medium.alpha_coeff );
        end
    end
end
        ...
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                if isscalar(medium.alpha_power)
                    ...
                    FFdrho=fftn(rho0 .* (duxdx + duydy + duzdz));
                    FFrho=fftn(rhox + rhoy + rhoz);
                    for yidx=1:numuniquepairs
                        T1 = c0.^2 .* ( ...
                            (rhox + rhoy + rhoz) ...
                            + absorb_tau(yidx) .* real(ifftn( absorb_nabla1(:,:,:,yidx) .* ffdrho )) ...
                            - absorb_eta(yidx) .* real(ifftn( absorb_nabla2(:,:,:,yidx) .* ffrho )) ...
                            );
                        p(Locations(:,:,:,j1)==1)=T1(Locations(:,:,:,j1)==1);
                    end
                end
