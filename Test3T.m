...
Nscale=sum(sum(sum(pos2+pos3+pos4+pos5)));
CoeffScale=sum(sum(sum(pos2*0.75+pos3*0.6+pos4*0.22+pos5*0.14)));
PowerScale=sum(sum(sum(pos2*1.5+pos3*1.01+pos4*1.15+pos5*1.21)));
...
medium.alpha_power = zeros([kgrid.Nx, kgrid.Ny,kgrid.Nz]) + PowerScale/Nscale; % only corresponds to the coeff is zero, and so reduces the number of y values
medium.alpha_coeff = zeros([kgrid.Nx, kgrid.Ny,kgrid.Nz])+ CoeffScale/Nscale;
...
[sensor_dataH] = kspaceFirstOrder3D(kgrid, medium, source, sensor,'PMLInside',false,'PlotSim',false);

save('Outputs\test3T.mat','sensor_dataH','phan')
