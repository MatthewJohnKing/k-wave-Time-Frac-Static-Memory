...
% 4: simulation using kspaceFirstOrder1DTF with CFL = 0.05
% =========================================================================
% SIMULATION
% =========================================================================
...
for loop = 1:3
    ...
    % medium.density(ceil(Nx/2) : end)= 2;
    % medium.density(5*Nx/8 : 7*Nx/8)= 2;
...
figure;
plot(kgrid.t_array, sensor_data(1,:), 'k-',kgrid.t_array, sensor_data(2,:), 'b-');
box on;
xlabel('time [s]');
ylabel('p [ ? ]');
...
ylabel('\alpha [dB/cm]');
if example_number==4
    title('Time Fractional Loss')
elseif example_number==1
    title('Second Order Fractional Laplacian Loss')
else
    title('Fractional Laplacian Loss')
end
