...
%
% Adaptations allow for non scalar values in medium.alpha_coeff and
% medium.alpha_power currently treats the varied cases as the no absorption
% and no dispersion case.
%
...
if ~isfield(medium, 'alpha_coeff') || all(medium.alpha_coeff(:) == 0)
    ...
    % temp1 = 0;
    % temp2 = 1;
    temp1 = max(medium.sound_speed(:)) * min(absorb_tau(:)) * kmax^(max(max(max(medium.alpha_power))) - 1);
    temp2 = 1 - min(absorb_eta(:)) * kmax^(max(max(max(medium.alpha_power))) - 1);
    dt_estimate = (temp1 + sqrt(temp1^2 + 4 * temp2)) / (temp2 * kmax * max(medium.sound_speed(:)));
