...
if isscalar(medium.alpha_power) && isscalar(medium.alpha_coeff)
    ...
    for yInd = 1:length(uniquePowerValues)
        ...
        s(:, Ind)=sT1(:, Ind);
        A(:, Ind)=AT1(:, Ind);
        absorb_TF_tau( Ind)= 2* c0(Ind) .* (medium.alpha_coeff( Ind)) ./ sin((uniquePowerValues(yInd)-1)*pi/2);
        for jl=1:medium.alpha_L
        ...
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                if ~isscalar(medium.alpha_power)
                    % psi = (expsdt .* psi - onemexp.*ovs2.*(rho0 .* duxdx - pml_x.*rhox).' );

                    psi = (expsdt .* psi + onemexp.*ovs2.*((rhox-rhoprev)./dt).' );
                    ...
                    % psi = (expsdt .* psi - onemexp.*ovs2.*(rho0 .* duxdx - log(pml_x).*rhox) );
                    psi = (expsdt .* psi + onemexp.*ovs2.*(rhox-rhoprev)/dt );
                    p = c0.^2 .* (rhox ...
                     + absorb_TF_tau .* psi*A ...
                     );

                    % if t_index/10==round(t_index/10)
                    % figure(13);
                    % subplot(2, 2, 1)
                    % imagesc(psi.');
                    % title('psi');
                    % subplot(2, 2, 2);
                    % plot((rhox-rhoprev)/dt)
                    % title('drhox/dt')
                    % subplot(2, 2, 3)
                    % plot(psi*A)
                    % title('psi*A');
