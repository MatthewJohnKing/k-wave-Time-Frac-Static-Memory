function [sensor_data, mem_usage] = kspaceFirstOrder3DTF(kgrid, medium, source, sensor, varargin)

% = Adapted from the kspacefirstorder3D ===================================
%
% ADAPTIONS:
...
%     should do so exactly according to kspaceFirstOrder3D. Additionally no
...
%     many Birk-Song points should be considered. 
%
% DESCRIPTION:
...
%       
% This function has been edited from kspaceFirstOrder3D which is part of the k-Wave Toolbox (http://www.k-wave.org)
...
if max(size(medium.alpha_coeff)~=size(medium.alpha_power))
    medium.alpha_coeff=castZeros([kgrid.Nx,kgrid.Ny,1])+medium.alpha_coeff;
    medium.alpha_power=castZeros([kgrid.Nx,kgrid.Ny,1])+medium.alpha_power;
end


switch equation_of_state
    case 'absorbing'
        if medium.alpha_L/20 ~= ceil(medium.alpha_L/20)
            disp('medium.alpha_L is not a multiple of 20, Increasing L to the next nearest multiple')
            medium.alpha_L=ceil(medium.alpha_L/20)*20;
        end
        L=medium.alpha_L;
        if isscalar(medium.alpha_power)
            [sF,AF]=BirkSongpts(L,medium.alpha_power-1);
            for label=1:L/20
                my_field = strcat('n',num2str(label));
                svalues.(my_field) = sF((label-1)*20+1:label*20);
                Avalues.(my_field) = AF((label-1)*20+1:label*20);
                expsdt.(my_field)= zeros(20,1);
                onemexp.(my_field)= zeros(20,1);
                ovs2.(my_field)= zeros(20,1);
                for jL=1:20
                    expsdt.(my_field)(jL)=double(exp(-kgrid.dt.*vpa(svalues.(my_field)(jL)).^2));
                    onemexp.(my_field)(jL)=double(1-exp(-kgrid.dt.*vpa(svalues.(my_field)(jL)).^2));
                    ovs2.(my_field)(jL)=double(vpa(svalues.(my_field)(jL)).^-2);
                end
                psi.(my_field) = zeros(20,kgrid.Nx, kgrid.Ny,kgrid.Nz);
            end
            ...
            absorb_TF_tau=castZeros([kgrid.Nx,kgrid.Ny,kgrid.Nz,1]);

            if isscalar(c0)
                c0=castZeros([kgrid.Nx,kgrid.Ny,kgrid.Nz])+c0;
            end

            for yInd = 1:length(uniquePowerValues)

                [sT,AT]=BirkSongpts(medium.alpha_L,uniquePowerValues(yInd)-1);
                sT1 = repmat(sT, [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                AT1 = repmat(AT.', [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                idx = medium.alpha_power == uniquePowerValues(yInd);
                absorb_TF_tau(idx)=2* c0(idx) .* (medium.alpha_coeff(idx)) ./ sin((uniquePowerValues(yInd)-1)*pi/2);
                for label=1:L/20
                    my_field = strcat('n',num2str(label));

                    if yInd==1

                        svalues.(my_field) = castZeros([20, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
                        Avalues.(my_field)=castZeros([20,kgrid.Nx,kgrid.Ny,kgrid.Nz]);
                        ovs2.(my_field)=castZeros([20, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
                        expsdt.(my_field)=castZeros([20, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
                        onemexp.(my_field)=castZeros([20, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
                        psi.(my_field)=castZeros([20, kgrid.Nx,kgrid.Ny,kgrid.Nz]);

                    end

                    svalues.(my_field)(:,idx) = sT1((label-1)*20+1:label*20,idx);
                    Avalues.(my_field)(:,idx) = AT1((label-1)*20+1:label*20,idx);
                    
                    expsdtT=zeros(1,20);
                    onemexpT=zeros(1,20);
                    ovs2T=zeros(1,20);
                    for zind=1:20
                        expsdtT(zind)=double(exp(-kgrid.dt.*vpa(sT((label-1)*20+zind)).^2));
                        onemexpT(zind)=double(1-exp(-kgrid.dt.*vpa(sT((label-1)*20+zind)).^2));
                        ovs2T(zind)=double(vpa(sT((label-1)*20+zind)).^-2);
                    end

                    expsdtT1=repmat(expsdtT.', [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                    expsdt.(my_field)(:,idx)=expsdtT1(:,idx);

                    onemexpT1=repmat(onemexpT.', [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                    onemexp.(my_field)(:,idx)=onemexpT1(:,idx);

                    ovs2T1=repmat(ovs2T.', [1, kgrid.Nx, kgrid.Ny,kgrid.Nz]);
                    ovs2.(my_field)(:,idx)=ovs2T1(:,idx);

                end

            end

            clear('ovs2T1','ovs2T','idx','sT','AT','sT1','AT1','onemexpT','onemexpT1','expsdtT1','expsdtT')
        end
end
    ...
    if ~flags.nonlinear
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                psi=psiupdate(psi,expsdt,onemexp,ovs2,rho0,rhox,rhoy,rhoz,rhoxprev,rhoyprev,rhozprev,dt,duxdx,duydy,duzdz);
                p=pundate(medium.alpha_power,c0,rhox,rhoy,rhoz,absorb_TF_tau,Avalues,psi);
        ...
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                psi = expsdt .* psi - (onemexp.*ovs2.*rho0 .* (duxdx + duydy +duzdz) );
                
                % calculate p using a linear absorbing equation of state          
                p = c0.^2 .* ( ...
                   (rhox + rhoy + rhoz) ...
                   + absorb_TF_tau .* permute(pagemtimes( A, permute(psi,[4,1,2,3])), [2,3,4,1]) ...
...
function psi=psiupdate(psi,expsdt,onemexp,ovs2,rho0,rhox,rhoy,rhoz,rhoxprev,rhoyprev,rhozprev,dt,duxdx,duydy,duzdz)
B=permute((rhox+rhoy+rhoz-rhoxprev-rhoyprev-rhozprev)/ dt,[4,1,2,3]);

for label=1:numel(fieldnames(psi))
    my_field = strcat('n',num2str(label));
    psi.(my_field) = expsdt.(my_field) .* psi.(my_field) + (onemexp.(my_field).*ovs2.(my_field).* B);
end
end

function p=pundate(power,c0,rhox,rhoy,rhoz,absorb_TF_tau,A,psi)
if isscalar(power)
    T1=zeros(size(rhox));
    for label=1:numel(fieldnames(psi))
        my_field = strcat('n',num2str(label));
        C=pagemtimes( A.(my_field),psi.(my_field));
        C1=permute(C, [2,3,4,1]);
        T1=T1 + C1;
    end
    p=c0.^2.* (rhox + rhoy + rhoz + absorb_TF_tau .* T1);
else
    T1=zeros(size(rhox));
    for label=1:numel(fieldnames(psi))
        my_field = strcat('n',num2str(label));
        C=dot( A.(my_field), psi.(my_field));
        C1=permute(C, [2,3,4,1]);
        T1=T1 + C1;
    end
    p = c0.^2 .* ( ...
