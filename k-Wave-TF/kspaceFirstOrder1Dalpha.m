function sensor_data = kspaceFirstOrder1Dalpha(kgrid, medium, source, sensor, varargin)

% = Adapted from the kspacefirstorder1D ===================================
%
% ADAPTIONS:
...
%
% USAGE:
%     sensor_data = kspaceFirstOrder1Dalpha(kgrid, medium, source, sensor)
%     sensor_data = kspaceFirstOrder1Dalpha(kgrid, medium, source, sensor, ...) 
%
...
if ~isscalar(medium.alpha_power)
    ...
    Locations=castZeros([kgrid.Nx,2]);
        ...
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                if isscalar(medium.alpha_power)
                    ...
                    Fdrho=fft(rho0 .* duxdx);
                    Frho =fft(rhox);
                    for Index = 1:numuniquepairs
                        my_field = strcat('Ind',num2str(Index));
                        T1=c0.^2 .* ( rhox ...
                            + absorb_tau.(my_field) .* real(ifft( absorb_nabla1.(my_field) .* Fdrho )) ...
                            - absorb_eta.(my_field) .* real(ifft( absorb_nabla2.(my_field) .* Frho )) ...
                            );
                        p(Locations(:,Index)==1)=T1(Locations(:,Index)==1);
                    end
        ...
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                if isscalar(medium.alpha_power)
                    ...
                    for Index = 1:numuniquepairs
                        T1=c0.^2 .* ( rhox ...
                                + absorb_tau(Index) .* real(ifft( absorb_nabla1(:,Index) .* Fdrho )) ...
                                - absorb_eta(Index) .* real(ifft( absorb_nabla2(:,Index) .* Frho )) ...
                                + medium.BonA .* rhox.^2 ./ (2 .* rho0) ...
                                );
                            p(Locations(:,Index)==1)=T1(Locations(:,Index)==1);
                    end
