function [sensor_data, mem_usage] = kspaceFirstOrder3DTFv2(kgrid, medium, source, sensor, varargin)
...
%
%     v2 makes use of a different appraoch to approaching varied
%     absorption, with sj and Aj x psij computed for each of the different
%     absorption values, then being pieced together to cover the domain.
%     this reduces memory usage but increases running time.
%
% USAGE:
%     sensor_data = kspaceFirstOrder3DTFv2(kgrid, medium, source, sensor)
%     sensor_data = kspaceFirstOrder3DTFv2(kgrid, medium, source, sensor, ...) 
%
    ...
    case 'absorbing'
        ...
        if isscalar(medium.alpha_power)
            ...
            for label=1:L/20
                ...
                svalues.(my_field) = vpa(sF((label-1)*20+1:label*20));
                Avalues.(my_field) = AF((label-1)*20+1:label*20);
                expsdt.(my_field)= zeros(20,1);
                onemexp.(my_field)= zeros(20,1);
                for jL=1:20
                    expsdt.(my_field)(jL)=double(exp(-kgrid.dt.*svalues.(my_field)(jL).^2));
                    onemexp.(my_field)(jL)=double(Avalues.(my_field)(jL).*(1-exp(-kgrid.dt.*svalues.(my_field)(jL).^2)).*(svalues.(my_field)(jL).^-2));
                end
                psi.(my_field) = zeros(20,kgrid.Nx, kgrid.Ny,kgrid.Nz);
            end
            absorb_TF_tau= 2* c0 .* (medium.alpha_coeff) ./ sin((medium.alpha_power-1)*pi/2);
            clear('svalues','Avalues','SF','AF')
        else
            ...
            idx=castZeros([kgrid.Nx,kgrid.Ny,kgrid.Nz]);
            expsdt=castZeros([L,length(uniquePowerValues)]);
            onemexp=castZeros([L,length(uniquePowerValues)]);

            for yInd = 1:length(uniquePowerValues)
                [sT,AT]=BirkSongpts(medium.alpha_L,uniquePowerValues(yInd)-1);
                idx1 = (medium.alpha_power == uniquePowerValues(yInd))*yInd;
                idx=idx+idx1;
                absorb_TF_tau(idx(:,:,:)==yInd)=2* c0(idx(:,:,:)==yInd) .* (medium.alpha_coeff(idx(:,:,:)==yInd)) ./ sin((uniquePowerValues(yInd)-1)*pi/2);
                if yInd==1
                    psi=castZeros([L, kgrid.Nx,kgrid.Ny,kgrid.Nz]);
                end
                expsdt(:,yInd)=double(exp(-kgrid.dt.*vpa(sT).^2));
                onemexp(:,yInd)=double((AT.').*(1-exp(-kgrid.dt.*vpa(sT).^2)).*(vpa(sT).^-2));
            end
            clear('sT','AT','idx1')
        end
end
    ...
    if ~flags.nonlinear
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                psi=psiupdate(psi,idx,expsdt,onemexp,rho0,rhox,rhoy,rhoz,rhoxprev,rhoyprev,rhozprev,dt,duxdx,duydy,duzdz);
                p=pundate(c0,rhox,rhoy,rhoz,absorb_TF_tau,psi);
...
function psi=psiupdate(psi,idx,expsdt,onemexp,rho0,rhox,rhoy,rhoz,rhoxprev,rhoyprev,rhozprev,dt,duxdx,duydy,duzdz)
B=(rhox+rhoy+rhoz-rhoxprev-rhoyprev-rhozprev)/ dt;

for ypower=1:length(expsdt(1,:))
    A=(idx==ypower);
    a=expsdt(:,ypower).* psi(:,A)+onemexp(:,ypower).* B(A).' ;
    psi(:,A)=a;
    clear('a')
end
end

function p=pundate(c0,rhox,rhoy,rhoz,absorb_TF_tau,psi)
