function [sensor_data, mem_usage] = kspaceFirstOrder2DTF(kgrid, medium, source, sensor, varargin) %#ok<STOUT>
% = Adapted from the kspacefirstorder2D ===================================
%
% ADAPTIONS:
%     This code has been adapted from kspaceFirstOrder2D to be able to run
...
%     a single value of alpha or one that changes over the dom ain. An
%     additional input of medium.alpha_L is required to resperesent how
%     many Birk-Song points should be considered. 
%
% USAGE:
%     sensor_data = kspaceFirstOrder2DTF(kgrid, medium, source, sensor)
%     sensor_data = kspaceFirstOrder2DTF(kgrid, medium, source, sensor, ...) 
%
...
switch equation_of_state
    case 'absorbing'
        if isscalar(medium.alpha_power)
            ...
            psi = zeros(medium.alpha_L,kgrid.Nx, kgrid.Ny);
        else

            if isscalar(c0)
                c0=castZeros([kgrid.Nx,kgrid.Ny])+c0;
            end

            uniquePowerValues = unique(medium.alpha_power(:));
            s=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny]);
            A=castZeros([medium.alpha_L,kgrid.Nx,kgrid.Ny]);
            ovs2=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny]);
            expsdt=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny]);
            onemexp=castZeros([medium.alpha_L, kgrid.Nx,kgrid.Ny]);
            absorb_TF_tau=castZeros([kgrid.Nx,kgrid.Ny,1]);

            for yInd = 1:length(uniquePowerValues)
                [sT,AT]=BirkSongpts(medium.alpha_L,uniquePowerValues(yInd)-1);
                sT1 = repmat(sT, [1, kgrid.Nx, kgrid.Ny]);
                AT1 = repmat(AT.', [1, kgrid.Nx, kgrid.Ny]);
                ...
                expsdtT1=repmat(expsdtT.', [1, kgrid.Nx, kgrid.Ny]);
                expsdt(:,idx)=expsdtT1(:,idx);

                onemexpT1=repmat(onemexpT.', [1, kgrid.Nx, kgrid.Ny]);
                onemexp(:,idx)=onemexpT1(:,idx);

                ovs2T1=repmat(ovs2T.', [1, kgrid.Nx, kgrid.Ny]);
                ovs2(:,idx)=ovs2T1(:,idx);
...
rhoxprev    = castZeros([kgrid.Nx, kgrid.Ny]);
    ...
    if ~flags.nonlinear
        switch equation_of_state
            ...
            case 'absorbing'
                ...
                psi=psiupdate(psi,expsdt,onemexp,ovs2,rho0,rhox,rhoy,rhoxprev,rhoyprev,dt);
        ...
        switch equation_of_state
            ...
            case 'absorbing'
                if isscalar(medium.alpha_power)
                    psi = expsdt .* psi - (onemexp.*ovs2.*rho0 .* (duxdx + duydy) );
                    % loss=castZeros([kgrid.Nx, kgrid.Ny]);
                    % for J=1:L
                    %     loss = loss + A(J)*psi(:,:,J);
                    % end
                    ...
                    p = c0.^2 .* ( ...
                        (rhox + rhoy) ...
                        + absorb_TF_tau .* permute(pagemtimes( A, permute(psi,[3,1,2])), [2,3,1]) ...
                    ...
                    p = c0.^2 .* ( ...
                        (rhox + rhoy) ...
                        + absorb_TF_tau .* permute(dot( permute(A,[3,1,2]), permute(psi,[3,1,2])), [2,3,1]) ...
                        + medium.BonA .* (rhox + rhoy).^2 ./ (2 .* rho0) ...
                        );
                end
...
function psi=psiupdate(psi,expsdt,onemexp,ovs2,rho0,rhox,rhoy,rhoxprev,rhoyprev,dt)
B=permute((rhox+rhoy-rhoxprev-rhoyprev)/ dt ,[3,1,2]);
