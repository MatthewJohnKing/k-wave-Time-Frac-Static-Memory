function [sensor_data, mem_usage] = kspaceFirstOrder2Dalpha(kgrid, medium, source, sensor, varargin) %#ok<STOUT>
...
%
% USAGE:
%     sensor_data = kspaceFirstOrder2Dalpha(kgrid, medium, source, sensor)
%     sensor_data = kspaceFirstOrder2Dalpha(kgrid, medium, source, sensor, ...) 
%
...
%       
% This function has been edited from kspaceFirstOrder2D which is part of the k-Wave Toolbox (http://www.k-wave.org)
...
if ~isscalar(medium.alpha_power)
    ...
    Locations=castZeros([kgrid.Nx,kgrid.Ny,2]);
            ...
            case 'absorbing'
                ...
                p = c0.^2 .* ( ...
                   ...
                   - absorb_eta .* real(ifft2( absorb_nabla2 .* fft2(rhox + rhoy) )) ...
                    ...
                    ffdrho=fft2(rho0 .* (duxdx + duydy));
                    ffrho=fft2(rhox + rhoy);

                    for j1=1:numuniquepairs
                        my_field = strcat('Ind',num2str(j1));
                        T1 = c0.^2 .* ( ...
                            (rhox + rhoy) ...
                            + absorb_tau.(my_field) .* real(ifft2( absorb_nabla1.(my_field) .* ffdrho )) ...
                            - absorb_eta.(my_field) .* real(ifft2( absorb_nabla2.(my_field) .* ffrho )) ...
                            );
            ...
            case 'absorbing'
                ...
                if isscalar(medium.alpha_power)
                    ...
                    for j1=1:numuniquepairs
                        T1 = c0.^2 .* ( ...
                            (rhox + rhoy) ...
                            + absorb_tau(j1) .* real(ifft2( absorb_nabla1(:,:,j1) .* ffdrho )) ...
                            - absorb_eta(j1) .* real(ifft2( absorb_nabla2(:,:,j1) .* ffrho )) ...
                            + medium.BonA .* (rhox + rhoy).^2 ./ (2 .* rho0) ...
                            );
                        p(Locations(:,:,j1)==1)=T1(Locations(:,:,j1)==1);
                    end
