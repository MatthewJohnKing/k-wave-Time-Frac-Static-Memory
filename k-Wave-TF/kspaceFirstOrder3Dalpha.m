...
%
% ADAPTIONS:
%     This code has been adapted from kspaceFirstOrder3D to be able to run
            ...
            case 'absorbing'
                ...
                if isscalar(medium.alpha_power)
                    ...
                    for yidx=1:numuniquepairs
                        my_field = strcat('Ind',num2str(yidx));
                        T1 = c0.^2 .* ( ...
                            (rhox + rhoy + rhoz) ...
                            + absorb_tau.(my_field) .* real(ifftn( absorb_nabla1.(my_field) .* FFdrho )) ...
                            - absorb_eta.(my_field) .* real(ifftn( absorb_nabla2.(my_field) .* FFrho )) ...
                            );
                        p(Locations(:,:,:,yidx)==1)=T1(Locations(:,:,:,yidx)==1);
                    end
                end
